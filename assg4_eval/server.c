...
int main()
{
	int sockfd, connfd, nb;
	char password[1024] = "1234";
	...
	while (1)
	{
		...
		{
			bzero(buff, 1024);
			nb = recv(connfd, buff, 1024, 0);
			// printf("%s\n", buff);
			if (strcmp(buff, password) == 0)
			{
				flag = 1;
				bzero(buff, 1024);
				strcpy(buff, "Welcome to project COBRA\n");
				char str[100];
				int live_count = 0;
				for (int j = 0; j < count; j++)
				{
					...
					{
						live_count++;
						sprintf(str, "%s is live\n", arr[j].Name);
						strcat(buff, str);
					}
				}
				if(live_count == 0)
				{
					strcat(buff, "No other Co-PIs connected.\n");
				}
				strcat(buff, "Enter name\n");
			...
			{
				strcpy(buff, "Password InCorrect. Try again:");
				send(connfd, buff, strlen(buff), 0);
			}
		}
	}
